...
export default function TrainerHome() {
  const [openJugadoras, setOpenJugadoras] = useState(false);
  ...
  const cards = [
    ...
    {
      title: "Prueba de salto",
      description: "Mide el rendimiento en salto y la explosividad.",
    ...
    {
      title: "Jugadoras",
      description: "Gestiona y consulta el perfil de tus jugadoras.",
      image: "/jugadores.png",
        ...
        >
          {cards.map((card, index) => (
            <li key={card.title} className="h-full">
              {card.isToggle ? (
                // Versión con toggle - sin Link exterior
                <div className="relative h-full">
                  <button
                    type="button"
                    onClick={() => setOpenJugadoras(!openJugadoras)}
                  ...
                  {openJugadoras && (
                    <div className="absolute inset-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm rounded-3xl flex items-center justify-center z-20 p-6">
                      <div className="w-full max-w-sm grid grid-cols-1 gap-3">
                        <Link
                          href="/jugadores"
                          onClick={() => setOpenJugadoras(false)}
                          className="block w-full px-4 py-3 rounded-xl text-center font-semibold text-white bg-[#B01C3B] hover:bg-[#8F1630] shadow-sm hover:shadow-md transition-transform duration-200 hover:scale-[1.02] active:scale-95 focus:outline-none focus:ring-2 focus:ring-[#B01C3B]/50"
                        >
                          Gestionar jugadoras
                        </Link>
                        <Link
                          href="/resultados-jugadora"
                          onClick={() => setOpenJugadoras(false)}
                          className="block w-full px-4 py-3 rounded-xl text-center font-semibold text-white bg-[#B01C3B] hover:bg-[#8F1630] shadow-sm hover:shadow-md transition-transform duration-200 hover:scale-[1.02] active:scale-95 focus:outline-none focus:ring-2 focus:ring-[#B01C3B]/50"
                        >
                          Resultados reacción
                        ...
                        <Link
                          href="/resultados-jugadores-salto"
                          onClick={() => setOpenJugadoras(false)}
                          className="block w-full px-4 py-3 rounded-xl text-center font-semibold text-white bg-[#B01C3B] hover:bg-[#8F1630] shadow-sm hover:shadow-md transition-transform duration-200 hover:scale-[1.02] active:scale-95 focus:outline-none focus:ring-2 focus:ring-[#B01C3B]/50"
                        >
                          Resultados salto
