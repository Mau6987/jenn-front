          ...
          >
            {options.slice(1).map(opt => (
              <button
                ...
                style={{
                  color: value === opt.key ? T.accent : T.muted,
      ...
      {slots.map(({ player, pos }) => {
        ...
        return (
          ...
          >
            {/* Nombre */}
            <p
              className="text-[10px] font-semibold text-center mb-2 line-clamp-2 leading-tight px-1 transition-colors group-hover:text-blue-600"
              style={{ color: T.muted }}
            >
              {player.nombre}
            </p>

            {/* Avatar con anillo de medalla */}
            <div className="relative mb-1.5 transition-transform duration-200 group-hover:scale-105">
              <Avatar
                className={`${avSize[pos]} border-[3px]`}
                style={{
                  borderColor: mc.ring,
                  boxShadow: `0 0 0 3px ${mc.bg}, 0 2px 10px rgba(0,0,0,0.08)`,
                }}
              >
            ...
            {/* Badge — fuerza total */}
            <div
              className="text-[9px] font-bold uppercase tracking-wide px-2.5 py-0.5 rounded-full mb-2"
              style={{
                background: mc.bg,
                color: mc.text,
            ...
            >
              {player.mejor_fuerza_total ?? "—"} kg
            </div>

            {/* Barra */}
            <div
              className="w-full rounded-t-xl flex items-center justify-center transition-opacity duration-200 group-hover:opacity-85"
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ background: T.pageBg, color: T.muted }}
      >
        {player.posicion}
      </span>
      ...
      <div className="text-right shrink-0">
        ...
        <p className="text-[10px] uppercase tracking-wide" style={{ color: T.mutedSoft }}>
          {player.mejor_altura ?? "—"} cm · {player.mejor_cantidad ?? "—"} saltos
        </p>
      </div>
    </motion.div>
  )
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-4 px-5 py-4" style={{ background: T.text }}>
            <div
              className="w-12 h-12 rounded-full grid place-items-center overflow-hidden"
            ...
            {player.posicion <= 3 ? (
              <span
                ...
                style={{ background: MEDAL[player.posicion].ring }}
              >
                #{player.posicion}
              </span>
            ) : (
          ...
          {/* Stats — igual estructura que el original, datos nuevos */}
          <div className="px-5 py-5 grid grid-cols-2 gap-3">
            {[
              { label: "Fuerza total",  value: `${player.mejor_fuerza_total ?? "—"} kg`, color: T.accent   },
              { label: "Altura",        value: `${player.mejor_altura       ?? "—"} cm`, color: "#0891b2"  },
  ...
  return (
    <div className="min-h-screen py-10 px-4" style={{ background: T.pageBg }}>
      <div className="max-w-xl mx-auto">
        ...
        {/* ── Dropdown — fuera del card, arriba izquierda ─────── */}
        <motion.div
          className="mb-6"
        ...
        {/* ── Título — fuera del card, centrado ───────────────── */}
        <motion.div
        ...
        >
          <span
            className="text-[11px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full text-white"
        ...
        >
          {/* Tabs período */}
          <div
            className="flex justify-center gap-0 py-3 px-4"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {data.length === 0 ? (
                <p className="text-center text-sm py-16" style={{ color: T.mutedSoft }}>
                  Sin datos disponibles.
                </p>
              ) : (
                <>
                  {top3.length > 0 && (
                    <Podium players={top3} onPlayerClick={setSelectedPlayer} />
                  )}
                  {rest.length > 0 && (
                    <div className="px-5 py-5 space-y-2">
                      {rest.map((player, i) => (
                        <RankRow
                          key={player.cuentaId || player.posicion}
                          player={player}
                          delay={i * 0.04}
