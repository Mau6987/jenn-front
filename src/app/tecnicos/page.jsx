...
export default function TecnicosPage() {
  ...
  const validateForm = () => {
    ...
    // Usuario: obligatorio siempre en crear y editar
    ...
    if (formMode === "create") {
      if (!formData.contraseña.trim()) errors.contraseña = "La contraseña es obligatoria"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formMode === "update" && !submitData.contraseña) {
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={handleOpenCreateForm} disabled={loading} className="flex items-center gap-2 px-5 py-2.5 bg-red-700 hover:bg-red-800 text-white text-sm font-semibold rounded-xl shadow-md shadow-red-200 transition-all hover:shadow-lg hover:shadow-red-200 disabled:opacity-50">
              ...
              {loading && !isModalOpen && !showDeleteModal ? (
                ...
                <div className="py-24 flex flex-col items-center gap-3 text-slate-400">
                  ...
                  <p className="font-semibold text-slate-600">{searchTerm ? "Sin resultados" : "No hay técnicos"}</p>
                  <p className="text-sm text-center max-w-xs">{searchTerm ? "Intenta con otros términos de búsqueda." : "Agrega tu primer técnico al sistema."}</p>
                </div>
              ) : (
                <>
                  <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                    {currentItems.map((tecnico) => (
                      <div key={tecnico.id} className="group relative bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-red-200 transition-all duration-300 overflow-hidden">
                        ...
                        <div className="flex items-center border-t border-slate-100 divide-x divide-slate-100">
                          <button onClick={() => handleViewTecnico(tecnico)} className="flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all">
                            <Eye className="h-3.5 w-3.5" /> Ver
                          </button>
                          <button onClick={() => handleEdit(tecnico)} className="flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold text-slate-500 hover:text-amber-600 hover:bg-amber-50 transition-all">
                            <Edit className="h-3.5 w-3.5" /> Editar
                          </button>
                          <button onClick={() => handleDeleteTecnico(tecnico)} className="flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all">
                    ...
                    <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          ...
                          return (
                            <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${currentPage === page ? "bg-red-700 text-white shadow-md shadow-red-200" : "text-slate-500 hover:bg-red-50 hover:text-red-700 border border-slate-200"}`}>
                              {page}
                            </button>
                          )
                        })}
                        <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-red-700 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed border border-slate-200 disabled:border-slate-100 transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[95vh] overflow-hidden border border-slate-200">
            ...
            <div className="bg-gradient-to-r from-red-800 to-red-700 px-8 py-6 flex items-center justify-between">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-red-200 hover:text-white p-2 hover:bg-white/10 rounded-xl transition-all">
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-7">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" name="nombres" value={formData.nombres} onChange={handleInputChange} disabled={formMode === "view"} placeholder="Ej: Juan Carlos" className={inputClass("nombres")} />
                    <FieldError field="nombres" />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-slate-700">Apellidos *</label>
                    <input type="text" name="apellidos" value={formData.apellidos} onChange={handleInputChange} disabled={formMode === "view"} placeholder="Ej: García López" className={inputClass("apellidos")} />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="email" name="correo_institucional" value={formData.correo_institucional} onChange={handleInputChange} disabled={formMode === "view"} placeholder="Ej: juan.garcia@universidad.edu" className={inputClass("correo_institucional")} />
                    <FieldError field="correo_institucional" />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-slate-700">Número de Celular *</label>
                    <input type="tel" name="numero_celular" value={formData.numero_celular} onChange={handleInputChange} disabled={formMode === "view"} placeholder="Ej: 71234567" className={inputClass("numero_celular")} />
                    <FieldError field="numero_celular" />
                  </div>

                  {/* ── Usuario: visible en todos los modos ── */}
                  <div className="space-y-1.5">
                  ...
                  {/* ── Contraseña: visible en crear y editar ── */}
                  {formMode !== "view" && (
                ...
                <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                  ...
                  {formMode !== "view" && (
                    <button type="submit" disabled={loading} className="px-6 py-2.5 bg-red-700 hover:bg-red-800 text-white font-semibold text-sm rounded-xl shadow-md shadow-red-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
