...
interface MicroController {
  ...
  label: string
  active: boolean
  lastResponse?: "acierto" | "error" | null
}

type TestMode = "secuencial" | "aleatorio" | "manual"
...
export default function PruebasPage() {
  ...
  const [testMode, setTestMode] = useState<TestMode>("secuencial")
  const [testStatus, setTestStatus] = useState<TestStatus>("idle")
  const [currentTest, setCurrentTest] = useState<any>(null)

  // Test configuration
  const [rounds, setRounds] = useState<number>(1)
  const [duration, setDuration] = useState<number>(60) // seconds for random mode
  ...
  const [results, setResults] = useState<TestResult>({ aciertos: 0, errores: 0 })
  ...
  const executeManualSequence = (sequences: string[]) => {
  ...
  const sendCommandToDevice = async (deviceId: number, command: string) => {
    try {
  ...
  const handleDeviceResponse = (response: "acierto" | "error") => {
              ...
              <Card className="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={testMode}
                      onValueChange={(value: TestMode) => setTestMode(value)}
                      disabled={testStatus === "running"}
                    >
                      <SelectTrigger className="w-full h-10 rounded-lg border border-gray-300 focus:border-red-500 focus:ring-1 focus:ring-red-500">
                        <SelectValue />
                  ...
                  {microControllers.map((mc) => (
                    ...
                    >
                      <div className="text-center">
                        ...
                        <div className="text-xs text-gray-500">Micro {mc.id}</div>
